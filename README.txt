...
\f0\fs24 \cf0 Conder Shou\
cs3544\
\
\'97How to run app\
\
Click on \'93index.html\'94\
...
\
\'97 Form (ADD HEURISTIC REFERENCES)\
\
- Default is TitleStartsWith since most cases, user doesn\'92t know exact name of comic, and in these cases the user would rather have false positives than false negatives \
	- easy toggle with Title to match title so that user can change to this mode if they want to search for comics for exact title (a more advanced option but likely the most common of the advanced options since we intuitively think of titles of works rather than the year/format)\
		- it\'92s a toggle because if it was another field: doesn\'92t make sense to want to match with a title name AND to want a separate set of comics that have a similar starting name\
\
...
\
- Loading\
	- Was going to do progress but API doesn\'92t provide progress of the query, so went with generic loading indicator\
